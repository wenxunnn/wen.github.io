...
const HINTS = {
  clue1: "The blanks are part of the lock. Some endings take more room than others.",
  clue2: "The examiner wrote down the temperature, but not the season. The missing month may matter more than the number itself.",
  clue3: "A truthful alibi should survive being placed on the same line as the keycard log.",
  clue4: "The clean summary is not the original record. One number was moved after the fact.",
};
...
const CHECKERS = {
  ...
  clue2: function (raw) {
    /*
      Body-temperature clue design:
      The website gives the autopsy temperatures and exam time, but NOT the
      cooling rate. Players must find the month in another case file, use your
      printed monthly cooling-rate prop, calculate the death time, then enter it.

      Suggested clean setup:
        Normal body temperature: 37.0 °C
        Body temperature at examination: 30.0 °C
        Medical examination time: 11:00 PM
        Month hidden elsewhere in evidence: June
        June cooling rate on physical prop: 1.00 °C/hour
        Death time: 4:00 PM
    */
    const cleaned = raw.trim().toLowerCase().replace(/\s+/g, "");

    // Accept clear 4 PM formats.
    if (["4", "4:00", "400"].includes(cleaned)) return true;

    const minutes = parseTimeToMinutes(raw);
    return minutes === 16 * 60; // 4:00 PM / 16:00
  },
